...
The starter uses the Next.js App Router. Pages and route handlers live in `app/`; reusable modules live in `lib/`. Database access uses the singleton in `lib/db.ts`, backed by Prisma's PostgreSQL adapter and a shared `pg` pool.
